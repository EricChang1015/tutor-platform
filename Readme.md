...
## 📅 預約系統增強 (2025-10-06 新增)
系統新增了完整的預約取消功能和改進的預約創建體驗：

### 新增功能
- **完整的預約創建 API**: 支援所有必要參數，包括教師ID、開始時間、時區等
- **預約取消功能**: 實現完整的取消政策，支援不同角色的取消規則
- **分層取消政策**:
  - 24小時前：免費取消
  - 12-24小時：扣除1張取消卡
  - 2-12小時：扣除2張取消卡
  - 2小時內：不允許取消
- **角色權限管理**: 學生、教師、管理員有不同的取消權限和政策

### API 端點
- `POST /bookings` - 創建預約（完整參數支援）
- `POST /bookings/{id}/cancel` - 取消預約
- `GET /bookings/{id}` - 查看預約詳情

### 測試功能
- **testAPI.html**: 新增預約取消測試案例
- **demo.html**: 新增預約取消用戶界面
- **完整測試覆蓋**: 包含跨時區預約和取消測試

## 💳 購買項目系統 (2025-10-07 新增)
系統新增了完整的課程卡片管理功能，支援多種卡片類型和自動化管理：

### 卡片類型
- **約課次卡**: 由管理員授予，激活後每張卡1週期限
- **體驗次卡**: 由管理員授予，效果等同約課次卡
- **補償次卡**: 由管理員授予，用於補償學生損失
- **取消約課次卡**: 每10堂課自動發放1張，用於取消課程

### 核心功能
- **卡片激活**: 需要激活才能使用，預設每張卡7天期限
- **自動扣除**: 預約課程自動扣除相應卡片（30分鐘1張）
- **智能退還**: 取消課程根據政策退還或扣除卡片
- **過期管理**: 自動檢查和標記過期卡片
- **自動發放**: 完成10堂課自動獲得1張取消卡

### 取消政策
- **24小時前**: 免費取消，退還課卡
- **12-24小時**: 退還課卡，扣除1張取消卡
- **2-12小時**: 退還課卡，扣除2張取消卡
- **2小時內**: 不允許取消，扣除課卡
- **教師取消**: 退還學生課卡
- **管理員取消**: 不扣除取消卡
...
### API 端點
- `GET /purchases` - 查看購買項目列表
- `POST /purchases` - 管理員創建購買項目
- `PUT /purchases/{id}` - 管理員更新購買項目
- `DELETE /purchases/{id}` - 管理員刪除購買項目
- `POST /purchases/{id}/activate` - 激活卡片
- `POST /purchases/{id}/extend` - 管理員延長過期時間

### 用戶界面
- **demo.html**: 新增"我的卡片"頁面，支援查看和激活卡片
- **testAPI.html**: 完整的 purchases API 測試案例
- **管理後台**: 支援創建和管理學生卡片
